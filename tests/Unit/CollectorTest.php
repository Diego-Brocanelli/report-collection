...
class CollectorTest extends TestCase
{
    ...
    {
        $collector = Collector::createFromArray($this->provider);
        $array = $collector->toArray();
    ...
    {
        $collector = Collector::createFromArray($this->provider);
        $collector->setInfoCreator('aaa');
        $collector->setInfoLastModifiedBy('bbb');
        $collector->setInfoTitle('ccc');
        $collector->setInfoSubject('ddd');
        $collector->setInfoDescription('eee');
        $collector->setInfoKeywords('fff');
        $collector->setInfoCategory('ggg');

        $temp_file = tempnam(sys_get_temp_dir(), 'CollectorInfoTest_') . ".xls";
        $collector->save($temp_file);
    ...
    public function testStyles()
    {
        ...
        $temp_file = tempnam(sys_get_temp_dir(), 'CollectorStylesTest_') . ".xls";
        $collector->save($temp_file);
        $this->assertFileExists($temp_file);
